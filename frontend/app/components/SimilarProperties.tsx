      ...
      {properties.map((p, i) => (
        <div key={i} className="prop-card border border-charcoal/10 bg-white overflow-hidden">
          ...
          <div className="relative h-40 bg-sand-dark overflow-hidden">
            <img
              src={errored[i] ? (FALLBACK[p.type] ?? FALLBACK.h) : p.image}
              alt={`${p.suburb} property`}
              className="w-full h-full object-cover"
              onError={() => setErrored(prev => ({ ...prev, [i]: true }))}
            />
            {/* badges */}
            <div className="absolute top-2 left-2">
              <span className="bg-charcoal text-sand font-mono text-[10px] px-2 py-0.5">
                {PROP_TYPES[p.type] || p.type}
              </span>
            </div>
            <div className="absolute top-2 right-2">
              <span className="bg-melb-blue text-white font-mono text-[10px] px-2 py-0.5">
                {Math.round(p.similarity_score * 100)}% match
              </span>
            </div>
            {/* price ribbon */}
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent px-3 py-2">
              <span className="font-mono text-sm font-bold text-white">{formatShort(p.price)}</span>
            </div>
          </div>
          ...
          <div className="p-3 space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <p className="font-mono text-xs text-charcoal/40">{p.distance.toFixed(1)} km CBD</p>
            </div>
            <div className="grid grid-cols-2 gap-x-3 font-mono text-[11px] text-charcoal/50">
              <span>🛏 {p.rooms} bed</span>
              <span>🏗 {p.building_area.toFixed(0)} m²</span>
              <span>🌿 {p.landsize.toFixed(0)} m² land</span>
