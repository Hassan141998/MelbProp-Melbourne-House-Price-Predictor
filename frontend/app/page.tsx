...
export default function Home() {
  ...
  useEffect(() => {
    API.getSuburbs().then(setSuburbs).catch(() => {});
    API.getModelAccuracy().then(setModelStats).catch(() => {});
  }, []);

  const handlePredict = useCallback(async (formData: any) => {
    setLoading(true);
    setError(null);
    try {
        ...
        <div className="max-w-screen-2xl mx-auto flex px-4">
          {TABS.map(({ id, label }) => (
            <button
              ...
              onClick={() => setTab(id)}
              className={[
                "font-mono text-xs uppercase tracking-widest px-5 py-3 transition-colors",
                tab === id
                  ? "bg-melb-blue text-white"
            ...
            <div className="xl:w-[60%] overflow-y-auto p-5 space-y-5">
              ...
              <div className="bg-white border border-charcoal/10 shadow-sm">
                ...
                <div className="p-5">
                  ...
                  {error && (
                    <p className="mt-4 p-3 bg-red-50 border-l-4 border-red-500 font-mono text-xs text-red-700">
                      {error}
                    </p>
                  )}
                </div>
              ...
              {modelStats && Object.keys(modelStats).length > 0 && (
                <div className="bg-white border border-charcoal/10 shadow-sm">
                  ...
                  <div className="p-5">
                    <ModelAccuracy stats={modelStats} />
                  </div>
                </div>
              )}
              ...
              {prediction?.similar_properties?.length > 0 && (
                <div className="fade-in-up bg-white border border-charcoal/10 shadow-sm">
                  <div className="bg-charcoal px-5 py-2.5">
                    <span className="font-mono text-xs uppercase tracking-widest text-sand">
                      5 Similar Properties (KNN)
                    </span>
                  </div>
                  <div className="p-5">
                    <SimilarProperties properties={prediction.similar_properties} />
                  </div>
                </div>
              )}
