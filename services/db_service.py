...
class DBService:
    ...
    async def _get_pool(self):
        ...
        if not self.pool:
            ...
            except Exception as e: print(f"[DB] Error: {e}"); self.pool=None
        return self.pool

    async def _create_tables(self):
        async with self.pool.acquire() as c:
            await c.execute("""CREATE TABLE IF NOT EXISTS predictions (
    ...
    async def save_prediction(self,d):
    ...
    async def get_history(self,limit=20):
        ...
        except Exception as e: print(f"[DB] Fetch error: {e}"); return []
