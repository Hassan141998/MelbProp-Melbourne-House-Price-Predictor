...
class KNNService:
    ...
    def _synthetic(self):
        ...
        return pd.DataFrame({
            "Suburb": np.random.choice(suburbs, n), "Rooms": np.random.randint(1,7,n),
    ...
    def find_similar(self, prop: dict, n: int = 5) -> list:
        ...
        q = np.array([[prop.get("rooms",3), prop.get("distance",10),
                       prop.get("building_area",150), prop.get("landsize",500),
                       prop.get("latitude",-37.81), prop.get("longitude",144.96)]], dtype=float)
        dists, idxs = self.knn.kneighbors(self.scaler.transform(q),
                                           n_neighbors=min(n+1, len(self.data)))
        ...
        for i, (d, idx) in enumerate(zip(dists[0][1:], idxs[0][1:])):
            ...
            yb = r.get("YearBuilt", 2000)
            out.append({
                "suburb": str(r.get("Suburb","?")),
                "rooms":  int(r.get("Rooms",0)),
                "type":   str(r.get("Type","h")),
                "distance": float(r.get("Distance",0)),
                "building_area": float(r.get("BuildingArea",0)),
                "landsize": float(r.get("Landsize",0)),
                "year_built": int(yb) if not pd.isna(yb) else 2000,
                "price": float(r.get("Price",0)),
                "similarity_score": round(1/(1+d), 3),
                "image": images[i % len(images)],
                "latitude":  float(r.get("Lattitude",-37.81)),
                "longitude": float(r.get("Longtitude",144.96)),
            })
        return out
