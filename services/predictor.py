...
class PricePredictor:
    ...
    def _X(self, df: pd.DataFrame) -> np.ndarray:
        return df[["Rooms","Type_enc","Method_enc","Distance",
    ...
    def _train_models(self):
        try:
            ...
            print(f"[MelbProp] Dataset loaded: {len(df)} rows")
        except FileNotFoundError:
            print("[MelbProp] No CSV found — using synthetic data")
            df = self._synthetic()

        df = self._prepare(df)
        X, y = self._X(df), df["Price"].values
    ...
    def _synthetic(self) -> pd.DataFrame:
        ...
        rooms = np.random.randint(1,7,n); dist = np.random.uniform(1,40,n)
        ba = np.random.uniform(50,450,n); land = np.random.uniform(0,1200,n)
        yb = np.random.randint(1900,2022,n)
    ...
    def predict(self, p: dict) -> dict:
        X = np.array([[p.get("rooms",3), self._enc("Type",p.get("type","h")),
                       ...
                       p.get("landsize",400), p.get("building_area",150),
                       p.get("year_built",2000), p.get("bathroom",1), p.get("car",1),
                       p.get("latitude",-37.81), p.get("longitude",144.96),
                       self._enc("Suburb",p.get("suburb",""))]], dtype=float)
        Xsc = self.scaler.transform(X)
        rf  = float(self.rf_model.predict(X)[0])
        xg  = float(self.xgb_model.predict(X)[0])
        lr  = float(self.lr_model.predict(Xsc)[0])
        pred = max(100_000, 0.5*rf + 0.35*xg + 0.15*lr)
        ...
        return {"predicted_price": round(pred),
                "price_range": {"low": round(pred*0.92), "high": round(pred*1.08)},
                "price_per_sqm": round(pred/ba), "confidence": conf,
                "model_comparison": {"random_forest": round(rf),
                                     "xgboost": round(xg),
                                     "linear_regression": round(lr)}}
