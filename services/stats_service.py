...
class StatsService:
    ...
    def _synthetic(self) -> pd.DataFrame:
        ...
        suburbs = [
            ...
            "Box Hill", "Dandenong", "Frankston", "Ringwood", "Geelong",
        ]
        years = np.random.choice([2013, 2014, 2015, 2016, 2017, 2018], n)
        sub_arr = np.random.choice(suburbs, n)
        base = {s: np.random.uniform(500_000, 2_000_000) for s in suburbs}
        prices = [
            base[s] * (1 + (y - 2013) * 0.06) + np.random.normal(0, 120_000)
            for s, y in zip(sub_arr, years)
        ]
        return pd.DataFrame({
            "Suburb":       sub_arr,
            "Price":        prices,
            ...
            "Landsize":     np.random.uniform(0, 1200, n),
            "Year":         years,
    ...
    def get_suburb_stats(self, suburb: str):
        ...
        for t in ["h", "u", "t"]:
            ...
            if not td.empty:
                by_type[t] = {"median": round(td.median()), "count": int(td.count())}
    ...
    def get_price_trends(self) -> list:
        ...
        return [
            {"year": int(r["Year"]), "median_price": round(r["Price"])}
