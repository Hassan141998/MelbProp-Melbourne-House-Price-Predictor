...
class StatsService:
    def __init__(self): self.df=None; self._load()

    def _load(self):
        try:
            df=pd.read_csv(DATA_PATH)
            df["Date"]=pd.to_datetime(df["Date"],dayfirst=True,errors="coerce")
            df["Year"]=df["Date"].dt.year; self.df=df
        except FileNotFoundError: self.df=self._synthetic()
    ...
    def _synthetic(self):
        ...
        base={s:np.random.uniform(500000,2000000) for s in suburbs}
        prices=[base[s]*(1+(y-2013)*0.06)+np.random.normal(0,120000) for s,y in zip(sub,years)]
        return pd.DataFrame({"Suburb":sub,"Price":prices,"Rooms":np.random.randint(1,7,n),
            "Type":np.random.choice(["h","u","t"],n),"Distance":np.random.uniform(1,40,n),
            "BuildingArea":np.random.uniform(50,400,n),"Landsize":np.random.uniform(0,1200,n),
            "Year":years,"Lattitude":np.random.uniform(-37.95,-37.65,n),
            "Longtitude":np.random.uniform(144.85,145.20,n)})

    def get_suburb_stats(self,suburb):
        ...
        return {"suburb":suburb,"count":int(len(sub)),
                "median_price":round(sub["Price"].median()),"mean_price":round(sub["Price"].mean()),
                "min_price":round(sub["Price"].min()),"max_price":round(sub["Price"].max()),
    ...
    def get_price_trends(self):
        ...
        t=(self.df.dropna(subset=["Price","Year"]).groupby("Year")["Price"]
